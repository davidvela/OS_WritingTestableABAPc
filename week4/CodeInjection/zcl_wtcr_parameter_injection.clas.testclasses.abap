...
CLASS ltd_cash_provider IMPLEMENTATION.
  ...
  METHOD zif_wtcr_cash_provider~get_notes.
    r_change = VALUE #( ( amount = 99 ) ( amount = 5 ) ( amount = 11 ) ).
  ENDMETHOD.
...
CLASS ltc_get_amount_in_coins DEFINITION FOR TESTING
  ...
  PRIVATE SECTION.
    DATA m_cut TYPE REF TO zcl_wtcr_parameter_injection.
...
CLASS ltc_get_amount_in_coins IMPLEMENTATION.
  ...
  METHOD amount_1_coin_1.
    "when
    DATA(coin_amount) = m_cut->get_amount_in_coins( i_amount = 1
                                                    i_cash_provider = NEW ltd_cash_provider( ) ). "parameter injection
  ...
  METHOD amount_2_coin_2.
    "when
    DATA(coin_amount) = m_cut->get_amount_in_coins( i_amount = 2
                                                    i_cash_provider = NEW ltd_cash_provider( ) ). "parameter injection
  ...
  METHOD amount_29_coin_4.
    "when
    DATA(coin_amount) = m_cut->get_amount_in_coins( i_amount = 29
                                                    i_cash_provider = NEW ltd_cash_provider( ) ). "parameter injection
