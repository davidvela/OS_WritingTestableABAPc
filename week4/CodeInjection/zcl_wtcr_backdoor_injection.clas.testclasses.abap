...
CLASS zcl_wtcr_backdoor_injection DEFINITION LOCAL FRIENDS ltc_get_amount_in_coins."back door injection
...
CLASS ltc_get_amount_in_coins DEFINITION FOR TESTING
  ...
  PRIVATE SECTION.
    DATA m_cut TYPE REF TO zcl_wtcr_backdoor_injection.
...
CLASS ltc_get_amount_in_coins IMPLEMENTATION.
  ...
  METHOD setup.
    "given
    m_cut = NEW #( ).
    m_cut->m_cash_provider = NEW ltd_cash_provider( ). "back door injection
  ENDMETHOD.
